...
def send_notification(email, domain, old_score, new_score, grade):
    try:
        ...
        msg["From"] = GMAIL_USER
        msg["To"] = email
        msg["Subject"] = f"CloudAudit — {domain} guvenlik skoru degisti!"
...
Eski Skor : {old_score}
Yeni Skor : {new_score}
Degisim   : {emoji} {'+' if diff > 0 else ''}{diff} puan
Seviye    : {grade}

Detayli rapor icin: http://localhost:5051
        """
        ...
        server.quit()
        print(f"📧 Bildirim gonderildi: {email}")
...
def run_scheduled_scan(app, domain, user_id, last_score):
    with app.app_context():
        ...
        print(f"⏰ Zamanlanmis tarama: {domain}")
        ...
        try:
            ...
            print(f"✅ Tamamlandi: {domain} — {score['total_score']}")
...
def add_scheduled_scan(app, domain, user_id, last_score, interval_hours=24):
    ...
    print(f"⏰ Eklendi: {domain} — her {interval_hours} saatte bir")

def remove_scheduled_scan(user_id, domain):
    job_id = f"scan_{user_id}_{domain}"
    if scheduler.get_job(job_id):
        scheduler.remove_job(job_id)
        print(f"⏰ Kaldirildi: {domain}")